...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header border-0 py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bolder text-dark">All Posts</span>
            <span class="text-muted mt-3 font-weight-bold font-size-sm">Total 0 Posts </span>
        </h3>
        <div class="card-toolbar">
            <a href="{{ route('post.add') }}" class="btn btn-primary font-weight-bolder font-size-sm" id="modal_toggle">
...
@section('js')
<script>
    $(document).on('change', '[name="post_type"]', function(e) {
        e.preventDefault();
        let elm = $(this);

        if (elm.val() == "poll") {
            $('.poll-post-fields').show();
        } else if (elm.val() == "job") {
            $('.job-post-fields').show();
        } else {
            $('.poll-post-fields').hide();
            $('.job-post-fields').hide();
        }
    });
    $(document).on('change', '[name="poll_option_count"]', function(e) {
        e.preventDefault();
        let elm = $(this);

        if (elm.val() == "" || elm.val() == undefined) {
            $(".poll-option-append").html("");
        } else {
            let html = '';
            ...
            for (let index = 1; index <= elm.val(); index++) {
                html += '<div class="row mb-2">'+
                            ...
                            '</div>'+
                            '<div class="col-10">'+
                                '<input type="text" class="form-control" name="poll_options[]" placeholder="Poll Option" autocomplete="off">'+
                            '</div>'+
                        '</div>';
            }
            $(".poll-option-append").html(html);
        }
    });
</script>
...
<script>
    var options = {
        ...
        columns: [{
                ...
                return ''+
                    '<div class="d-flex align-items-center">'+
                        '<div class="symbol symbol-40 symbol-sm flex-shrink-0">'+
                            '<img class="" src="'+base_url+'/'+row.post_images[0].path+'" alt="photo">'+
                        '</div>'+
                        '<div class="ml-4">'+
                            '<a href="#" class="text-dark-75 text-hover-primary font-weight-bolder font-size-lg mb-0">'+fn(row.title, '15')+'</a>'+
                            '<div class="text-muted font-weight-bold">'+type[row.post_type].type+'</div>'+
                        '</div>'+
                    '</div>';
            ...
            template: function(row) {
                return ''+
                    '<a href="'+base_url+'/posts/edit/'+row.id+'" class="btn btn-sm btn-clean btn-icon mr-2 edit-item" title="Edit details">'+
                        ...
                        '</span>'+
                    '</a>'+
                    '<a href="'+base_url+'/posts/delete/'+row.id+'" class="btn btn-sm btn-clean btn-icon delete-item" title="Delete">'+
                        '<span class="svg-icon svg-icon-md">'+
                            '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">'+
                                '<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">'+
                                    '<rect x="0" y="0" width="24" height="24"/>'+
